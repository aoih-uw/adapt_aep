function [p, cur_data, cur_data_sem]  = param_softplus(cur_data, cur_data_sem, amp_vec, noise_floor, lower_asy,sub_in_quad)
...
if sub_in_quad
    cur_data_pow = max(cur_data - noise_floor, 0);
    floor_val = 0.01 * noise_floor;                       % scale-appropriate floor
    cur_data_sem = cur_data_sem ./ (2*sqrt(cur_data_pow + floor_val));
    cur_data = sqrt(cur_data_pow);
end
