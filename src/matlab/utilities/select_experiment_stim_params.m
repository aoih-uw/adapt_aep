...
% Calculate the ms duration of the on/off ramp and full amplitude portion
% of the AEP stimulus, snapped to whole cycles
...
cur_full_amp_dur_samps = round(period_s*full_amp_cycles*fs);
...
cur_ramp_samples = round(period_s*ramp_cycles*fs);
cur_ramp_samples = max(cur_ramp_samples, round(50/1e3*fs)); % 50 ms into samples
ramp_duration_samples = ceil(cur_ramp_samples/period_samps)*period_samps; % Round up to nearest cycle
