...
%%% call main calibration function without correction factors (not yet
%%% determined), or intentionally being redetermined...
...
%%%
%%%
% update GUI with calibration data ... round so that display is clean.
...
% save into calibration_data (redundant with calibration_main, but that's ok)
...
% --- Executes on button press in butt_go_check_calibration.
...
ylabel('Voltage (0.1V/Pa)','Fontweight','bold','Fontsize',10);
xlabel('Time (s)','Fontweight','bold','Fontsize',10);  
% call main calibration function again ... correction factors etc now
% known...
if strcmp(calibration_data.experiment,'Stereo AM');
    cal_data = calibration_main_2ch(cax,tax,calibration_data);
else
...
% --- Executes on button press in butt_go_load_calibration.
function butt_go_load_calibration_Callback(hObject, eventdata, handles)
global calibration_data
global ghandles
cwd=cd;
cd('C:\Users\AEP\Desktop\Experiments\adaptiveAEP_2025\calibration\calibrationData');
[fname,~,~]=uigetfile('*.mat','Select a calibration file');
load(fname);
cd(cwd);
calibration_data = cal_data;
% Now load all previously generated calibration data into GUI display fields ...
...
% format data plot axis according to loaded calibration_data
